...
impl Ord for Utxo {
    fn cmp(&self, other: &Self) -> Ordering {
        // Sort in decreasing order of amount, and then in increasing order of
        // `script_pubkey` and `outpoint`.
        //
        // The rationale is that the coin selection algorithm uses largest-first
        // coin selection (as used in `bdk_wallet`). We then sort in increasing
        // order of `script_pubkey` to help optimize fees.
        other
            .amount
            .cmp(&self.amount)
            .then_with(|| self.script_pubkey.cmp(&other.script_pubkey))
