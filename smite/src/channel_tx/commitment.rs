...
pub enum CommitmentError {
    ...
    /// HTLC referenced by id was not found in the pending set.
...
impl ChannelConfig {
    ...
    fn build_sorted_nondust_htlc_outputs(
        ...
        outputs.sort_by(|a, b| {
            a.0.value
                ...
                .then_with(|| a.1.cltv_expiry.cmp(&b.1.cltv_expiry))
        });
    ...
    fn build_htlc_txs(
        ...
        for &nondust_htlc in &commitment.nondust_htlcs {
            ...
            // The HTLC transaction pays into a revocable output identical in
            // shape to `to_local`: spendable by the local party after the
            // remote's `to_self_delay`, or immediately via the revocation key.
            let output = TxOut {
            ...
            let tx = Transaction {
                version: Version::TWO,
                // An offered HTLC yields an HTLC-timeout transaction locked
                // until the HTLC's `cltv_expiry`; a received HTLC yields an
                // HTLC-success transaction with no locktime.
