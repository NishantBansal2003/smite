//! Funding-flow program scenario: deserialize a postcard `Program` from the
//! fuzz input and execute it against the target over an established encrypted
//! connection.
//!
//! Structurally identical to [`super::ir::IrScenario`]; kept as a separate
//! scenario so that the funding-flow fuzz harness has its own binary, corpus,
//! and mutator pairing.
...
/// Scenario that executes funding-flow IR programs against a target over an
/// encrypted connection established by `S`.
pub struct FundingFlowScenario<T: Target, S: SnapshotSetup<T>> {
    target: T,
...
impl<T: Target, S: SnapshotSetup<T>> Scenario for FundingFlowScenario<T, S> {
    ...
    fn run(&mut self, input: &[u8]) -> ScenarioResult {
        ...
        log::debug!(
            "[{:?}] Executing funding-flow program ({} instructions, {} input bytes)",
            start.elapsed(),
