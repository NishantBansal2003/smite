#!/bin/bash
#
# Generate an HTML coverage report for Eclair from a corpus of fuzz inputs.
#
# Usage: ./scripts/eclair-coverage-report.sh <corpus-dir> [output-dir]
#
# This script:
# 1. Builds (if needed) a coverage-instrumented Docker image
# 2. Runs each corpus input through eclair-scenario with JaCoCo coverage
# 3. Merges exec files and generates an HTML report
#
set -eu
...
if [ $# -lt 1 ]; then
    ...
    echo "  output-dir  Output directory for coverage report (default: ./eclair-coverage-report)"
    echo ""
...
# as named volumes
CORPUS_DIR="$(cd "$1" && pwd)"
OUTPUT_DIR="${2:-./eclair-coverage-report}"
...
if [ "${REBUILD:-}" = "1" ] || ! docker image inspect smite-eclair-coverage >/dev/null 2>&1; then
    SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
    SMITE_DIR="$(dirname "$SCRIPT_DIR")"

    echo "Building coverage Docker image..."
    docker build -t smite-eclair-coverage -f "$SMITE_DIR/workloads/eclair/Dockerfile.coverage" "$SMITE_DIR"
fi

# Create output directories (remove old data to avoid mixing with previous runs)
rm -rf "$OUTPUT_DIR/covdata" "$OUTPUT_DIR/html"
mkdir -p "$OUTPUT_DIR/covdata"

# Runs a single corpus input through the Eclair scenario.
#
# JaCoCo writes exec files atomically on JVM shutdown (via shutdown hook).
# The scenario sends SIGTERM to Eclair, which triggers the JVM to run shutdown
# hooks before exiting. We retry on failure since Docker containers can fail to
# start under high parallel load.
#
...
# each input gets its own covdata subdirectory to avoid filename collisions.
run_input() {
    local i="$1"
    local input_name="$2"
    ...
    for ((attempt=0; attempt<max_retries; attempt++)); do
        ...
        docker run --rm --user "$(id -u):$(id -g)" \
            ...
            smite-eclair-coverage \
            /eclair-scenario >/dev/null 2>&1 || true

        # Success if JaCoCo produced an exec file
        if [ -f "$covdir/jacoco.exec" ]; then
            return 0
        fi
...
if [ ! -f "$OUTPUT_DIR/covdata/input-0/jacoco.exec" ]; then
...
# Merge exec files and generate report
docker run --rm --user "$(id -u):$(id -g)" \
    -v "$OUTPUT_DIR:/output" \
    smite-eclair-coverage \
    sh -c '
        ...
        # Build merge command arguments from the file list
        MERGE_ARGS=""
        while IFS= read -r f; do
            MERGE_ARGS="$MERGE_ARGS $f"
        done < /tmp/exec-list.txt

        # Merge all exec files into a single file
        ...
        # Only pass fr.acinq JARs to --classfiles; scanning all of /opt/eclair/lib
        # fails because some dependency JARs (e.g. BouncyCastle) have multi-release
        # class entries that JaCoCo cannot handle.
        CLASSFILES=""
