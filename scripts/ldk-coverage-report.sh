#!/bin/bash
#
# Generate an HTML coverage report for LDK from a corpus of fuzz inputs.
#
# Usage: ./scripts/ldk-coverage-report.sh <corpus-dir> [output-dir]
#
# This script:
# 1. Builds (if needed) a coverage-instrumented Docker image
# 2. Runs each corpus input through ldk-scenario
...
if [ $# -lt 1 ]; then
    ...
    echo "  output-dir  Output directory for coverage report (default: ./ldk-coverage-report)"
    echo ""
...
# as named volumes
CORPUS_DIR="$(cd "$1" && pwd)"
OUTPUT_DIR="${2:-./ldk-coverage-report}"
...
if [ "${REBUILD:-}" = "1" ] || ! docker image inspect smite-ldk-coverage >/dev/null 2>&1; then
    SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
    SMITE_DIR="$(dirname "$SCRIPT_DIR")"

    echo "Building coverage Docker image..."
    docker build -t smite-ldk-coverage -f "$SMITE_DIR/workloads/ldk/Dockerfile.coverage" "$SMITE_DIR"
fi
...
# Runs a single corpus input through the LDK scenario.
#
...
run_input() {
    ...
    for ((attempt=0; attempt<max_retries; attempt++)); do
        ...
        docker run --rm --user "$(id -u):$(id -g)" \
            ...
            -e LLVM_PROFILE_FILE="/profdata/default.profraw" \
            smite-ldk-coverage \
            /ldk-scenario >/dev/null 2>&1 || true
...
docker run --rm --user "$(id -u):$(id -g)" \
    ...
    sh -c '
        ...
        llvm-cov show \
            /usr/local/bin/ldk-node-wrapper \
